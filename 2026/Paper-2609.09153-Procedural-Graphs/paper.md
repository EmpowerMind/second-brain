...
![Figure 1: Knowledge Graph triplets compared with Procedural Graph triplets.](./images/figure-1-knowledge-to-procedure.svg)
...
$$
\mathcal{G}=\big(\mathcal{V},\,\mathcal{R},\,\mathcal{E},\,\Phi\big),\qquad\mathcal{E}\subseteq\mathcal{V}\times\mathcal{R}\times\mathcal{V},
$$
...
$$
u_{t}=\mathrm{Match}\big(a_{t-1},\,\mathcal{V}\big),\qquad\mathcal{G}_{t}=\begin{cases}\mathcal{N}_{h}(u_{t}),&u_{t}\neq\varnothing,\\
\mathcal{G},&\text{otherwise,}\end{cases}\qquad g_{t}=\Psi\big(\mathcal{G}_{t},\;q,\;\mathcal{T}_{t-w:t}\big),
$$
...
$$
a_{t}\sim P_{\text{solver}}\big(\cdot\;\big|\;q,\;\mathcal{T}_{t},\;g_{t}\big).
$$
...
$$
S_{\text{val}}(\mathcal{G})=\frac{1}{|\mathcal{D}_{\text{val}}|}\sum_{(q,y)\in\mathcal{D}_{\text{val}}}S\big(f_{\text{solver}}(q\mid\mathcal{G}),\;y\big).
$$
...
$$
\mathcal{G}_{k}=\begin{cases}\mathcal{G}_{k}^{\text{cand}},&\text{if }S_{\text{val}}(\mathcal{G}_{k}^{\text{cand}})\geq S_{\text{val}}(\mathcal{G}_{k-1})\\
\mathcal{G}_{k-1},&\text{otherwise}\end{cases}.
$$
...
$$
\Delta\mathcal{G}_{k+1}\sim P_{\text{refiner}}\big(\cdot\;\big|\;\mathcal{G}_{k},\;\mathcal{C}_{k+1},\;\mathcal{H}_{\text{rejected}}\big).
$$
...
| | HotpotQA | MultiChallenge | GDPval | ALFWorld | $\tau$-bench | BFCL v3 |
|---|---|---|---|---|---|---|
...
| $\tau$-bench (Yao et al., 2024) | Customer service | 500 | 115 | Test uses the *retail* domain. |
| BFCL v3 (Patil et al., 2025) | Function calling | 100 | 100 | Base-category multi-turn tasks. |
| EnterpriseArena (Han et al., 2026) | Financial simulation | 50 | 50 | Simulator episodes under disjoint random-seed sets. The self-evolution study (Table 11) uses a smaller $20/20/20$ configuration. |
...
| RAP (Kagaya et al., 2024) | raw trajectories | similarity index | top-$k$ in-context exemplars |
| ExpeL (Zhao et al., 2024) | natural-language insights | unordered list | injected into system prompt |
| AutoGuide (Fu et al., 2024) | "in state $X$, do $Y$" guidelines | condition-indexed pool | state-matched retrieval |
...
| | HotpotQA | MultiChallenge | GDPval | ALFWorld | $\tau$-bench | BFCL v3 | EnterpriseArena |
|---|---|---|---|---|---|---|---|
...
$$
\mathcal{S}_{t}=\big(C_{t},\,L_{t},\,A_{t},\,IR_{t},\,PR_{t},\,AP_{t},\,D_{t},\,E_{t},\,U_{t}\big),
$$
...
1. Loan Amortization: A fraction of the loan portfolio $L_{t}$ matures, generating principal payments and interest income based on the lending rate.
2. User and Operational Costs: The active user base $U_{t}$ grows or decays organically. Fixed operational costs and user acquisition costs are deducted from the cash balance $C_{t}$.
3. Write-Offs: Defaulted loans are written off against the allowance $A_{t}$, and new provisions are calculated.
...
$$
s_{i,t}=\begin{cases}0,&\text{if run }i\text{ has gone bankrupt},\\
5\,\mathrm{Rev}^{(12)}_{i,t}+C_{i,t}-5{,}000\,N_{i,t},&\text{otherwise},\end{cases}
$$
...
*Table 11: PG self-evolution across ten rounds with Gemini 3.5 Flash. The $\Delta$ rows report metric changes relative to the previous best validation checkpoint. Gray rows mark rounds without a committed graph update.*
...
| $\Delta$ vs. Base | +45.0 | +54.10 | +$10.217M | – | +60.0 | +45.0 | -9.31 | +54.2 | +$40.21M |
...
| $\Delta$ vs. R01 | +35.0 | +23.70 | +$7.444M | – | +25.0 | +35.0 | -4.84 | +23.7 | +$11.65M |
...
| $\Delta$ vs. R02 | -15.0 | -10.45 | -$2.073M | – | -10.0 | -10.0 | +0.22 | -10.4 | -$5.59M |
...
| $\Delta$ vs. R02 | -5.0 | -3.40 | +$3.793M | – | – | -5.0 | -0.03 | -3.4 | +$18.52M |
| Round 05 | | | | | | | | | |
...
| $\Delta$ vs. R02 | -15.0 | -3.85 | -$2.207M | – | -5.0 | – | -0.17 | -3.8 | +$2.21M |
...
| $\Delta$ vs. R02 | – | +1.55 | +$1.569M | – | +5.0 | – | +0.04 | +1.6 | +$16.58M |
...
| $\Delta$ vs. R07 | +10.0 | +7.05 | +$9.764M | – | – | +10.0 | +0.01 | +7.0 | +$25.41M |
...
| $\Delta$ vs. R08 | – | – | -$0.801M | – | – | – | -0.05 | – | -$1.27M |
...
| $\Delta$ vs. R09 | -5.0 | – | -$0.184M | – | – | – | -0.07 | – | +$2.71M |
...
*Source: [arXiv:2609.09153v1](https://arxiv.org/abs/2609.09153) — converted from the paper's full text. Figures are linked to the arXiv-hosted images; algorithm listings, prompt-template boxes, and verbatim trace blocks from the appendices are not reproduced here.*
